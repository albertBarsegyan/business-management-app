...
export function PublicLandingPage({ bookingSlot }: { bookingSlot: ReactNode }) {
  ...
  const { pageBg, ink, inkMuted, hairline, gutter, accent, coverBg, dark, nav } = state;
  ...
  return (
    <div style={{ minHeight: "100vh", background: pageBg, color: ink }}>
      <div style={{ maxWidth: 1440, margin: "0 auto", background: pageBg }}>
        <div style={{ position: nav.sticky ? "sticky" : "static", top: 0, zIndex: 20 }}>
        ...
        <div style={{ padding: `26px ${gutter}`, background: bookingBg, borderTop: `1px solid ${hairline}`, display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            <span style={{ fontFamily: "var(--font-zhamo-display)", fontSize: "clamp(22px, 4vw, 24px)", lineHeight: 1.06, letterSpacing: "-0.022em", fontWeight: 700, color: ink }}>
              Book an appointment
            </span>
            <span style={{ fontSize: 11.5, color: inkMuted, whiteSpace: "nowrap" }}>No account needed</span>
          </div>
          {bookingSlot}
        </div>
