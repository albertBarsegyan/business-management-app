...
export function PreviewHero({ state }: { state: LandingBuilderState }) {
  ...
  // No backend source for trust metrics (rating, review count, years open)
  // yet — the toggle is kept (and still saved) but renders nothing rather
  // than fabricated numbers until one exists.
  const trustStrip = () => null;

  return (
    <div
      onClick={state.selectHero}
      style={{
    ...
    >
      ...
      {hero.layout === "split" && (
        <div
          ...
          style={{
            display: "grid",
            gridTemplateColumns: state.device === "phone" ? "1fr" : "1.1fr 1fr",
            gap: 20,
            padding: gutter,
            minHeight: heroHeight,
            alignItems: "center",
