...
export function BuilderTopbar({ state }: { state: LandingBuilderState }) {
  ...
  const publishLabel = state.isSaving
    ? "Saving…"
    : state.published
      ? "Published"
      : "Publish changes";
  const publishDisabled = state.isSaving || state.published;
  const publishBg = publishDisabled ? "#EDEFF2" : "#FFC935";
  const publishColor = publishDisabled ? "#A9AEB6" : "#17170F";
  ...
  return (
    ...
    >
      ...
      {state.saveError && (
        <span style={{ fontSize: 11.5, color: "#C7302F" }}>
          {state.saveError}
        </span>
      )}
      <div
      ...
      >
        ...
        <button
          className="zhamo-landing-viewlive"
