import { backOfficeNavItems } from "../model/data";

export function BackOfficeSidebar({ accent }: { accent: string }) {
  return (
    <aside style={{ width: 216, flex: "0 0 auto", background: "#14141A", display: "flex", flexDirection: "column" }}>
      ...
      <nav style={{ flex: "1 1 auto", overflowY: "auto", padding: "10px 8px", display: "flex", flexDirection: "column", gap: 2 }}>
        {backOfficeNavItems.map((n) => (
          <div
            key={n.label}
            className="zhamo-landing-navitem"
            style={{
              display: "flex",
              alignItems: "center",
              gap: 10,
              height: 34,
              padding: `0 8px 0 ${n.depth ? "38px" : "8px"}`,
              borderRadius: 6,
              cursor: "pointer",
              position: "relative",
              background: n.active ? "rgba(255,255,255,0.1)" : n.expanded ? "rgba(255,255,255,0.05)" : "transparent",
              color: n.active || n.expanded ? "#FFFFFF" : "#A7ADB8",
              fontSize: n.depth ? 12.5 : 13,
              fontWeight: n.active ? 600 : 400,
            }}
          >
            <span style={{ position: "absolute", left: -8, top: 7, bottom: 7, width: 3, borderRadius: "0 2px 2px 0", background: n.active ? accent : "transparent" }} />
            {!n.depth && (
              <span
                style={{
                  width: 20,
                  ...
                  fontSize: 10,
                  background: n.expanded ? accent : "rgba(255,255,255,0.07)",
                  color: n.expanded ? "#FFFFFF" : "#7C818B",
                }}
              >
                {n.mono}
              </span>
      ...
      <style>{`.zhamo-landing-navitem:hover { background: rgba(255,255,255,0.06); }`}</style>
    </aside>
  );
}
