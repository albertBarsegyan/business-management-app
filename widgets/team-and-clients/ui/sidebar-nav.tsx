import { buildNav, type ScreenKey } from "../model/nav";

export function SidebarNav({ screen }: { screen: ScreenKey }) {
  const nav = buildNav(screen);
  return (
    <aside style={{ width: 224, flex: "0 0 auto", background: "#14141A", display: "flex", flexDirection: "column" }}>
      ...
      <nav style={{ flex: "1 1 auto", overflowY: "auto", padding: "10px 8px", display: "flex", flexDirection: "column", gap: 2 }}>
        {nav.map((n) => (
          <div
            key={n.label}
            className="zhamo-teamclients-nav-item"
            style={{
              display: "flex",
              alignItems: "center",
              gap: 10,
              height: 34,
              padding: `0 8px 0 ${n.indent}`,
              borderRadius: 6,
              cursor: "pointer",
              position: "relative",
              background: n.bg,
          ...
          >
            ...
            {n.caret ? <span style={{ marginLeft: "auto", fontSize: 9, color: "#7C818B" }}>▾</span> : null}
          </div>
        ))}
      </nav>
      <div style={{ flex: "0 0 auto", borderTop: "1px solid rgba(255,255,255,0.07)", padding: "12px 10px 14px", display: "flex", flexDirection: "column", gap: 10, background: "#1A1A21" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          <span style={{ width: 28, height: 28, borderRadius: "50%", background: "oklch(0.64 0.16 350)", color: "#FFFFFF", fontSize: 11, fontWeight: 600, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
            LS
          </span>
          ...
          <span title="Log out" style={{ marginLeft: "auto", width: 24, height: 24, borderRadius: 5, background: "rgba(255,255,255,0.06)", color: "#A7ADB8", fontSize: 11, display: "inline-flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            ⏻
          </span>
        </div>
        <button style={{ height: 36, border: 0, borderRadius: 6, background: "oklch(0.64 0.16 350)", color: "#FFFFFF", fontFamily: "inherit", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          Administration
        </button>
      </div>
      <style>{`.zhamo-teamclients-nav-item:hover { background: rgba(255,255,255,0.06); }`}</style>
    </aside>
  );
}
