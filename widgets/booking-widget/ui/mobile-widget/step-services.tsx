  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      {flow.categories.map((c) => (
        <div key={c.name} style={{ borderBottom: "1px solid #EEF0F2", background: "#FFFFFF" }}>
          <div style={{ minHeight: 52, padding: "14px 16px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
              ...
              {c.services.map((s) => (
                ...
                >
                  ...
                  <span style={{ display: "flex", flexDirection: "column", gap: 2, flex: 1, minWidth: 0 }}>
                    ...
                    <span style={{ fontSize: 12, color: "#8A9099" }}>{s.duration}</span>
                  </span>
                  <span style={{ fontFamily: "var(--font-zhamo-mono)", fontSize: 13, fontWeight: 500, whiteSpace: "nowrap" }}>{s.price}</span>
                </div>
              ))}
            </div>
          )}
